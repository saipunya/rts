        ...
        <div class="col-12 col-md-6">
            <div class="card h-100 shadow-sm">
                <div class="card-body">
                    ...
                    <p class="card-text small text-muted mb-2">ส่งออกข้อมูลรายการเป็น PDF ตามรอบราคายาง</p>
                    <?php
                    // ดึงรอบวันที่จากราคายาง
                    require_once 'config.php';
                    $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
                    $periods = [];
                    if (!$conn->connect_error) {
                        $sql = "SELECT id, period_start, period_end FROM price ORDER BY period_start DESC";
                    ...
                    <form class="row g-2 align-items-end" method="get" action="export_rubbers_export.php" id="exportForm">
                        <div class="col-12">
                            <label for="period_id" class="form-label mb-1">เลือกรอบวันที่ (ตามราคายาง)</label>
                            <select class="form-select form-select-sm" id="period_id" name="period_id" required>
                                <option value="">-- เลือกรอบวันที่ --</option>
                                <?php foreach($periods as $p): ?>
                                    <option value="<?php echo $p['id']; ?>">
                                        <?php echo date('d/m/Y', strtotime($p['period_start'])) . ' - ' . date('d/m/Y', strtotime($p['period_end'])); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-12 d-flex gap-2">
                            <button type="submit" class="btn btn-success btn-sm"><i class="bi bi-file-earmark-arrow-down me-1"></i>ส่งออก PDF ตามรอบ</button>
                        </div>
                    </form>
                </div>
            </div>
